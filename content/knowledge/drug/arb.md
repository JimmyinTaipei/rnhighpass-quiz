---
title: 血管張力素 II 受體阻斷劑
subtitle: Angiotensin II receptor blockers(-sartan)
aliases: [ARB, Angiotensin receptor blocker, 血管收縮素 II 受體拮抗劑, Losartan, Valsartan, Irbesartan, Candesartan, Telmisartan, Olmesartan, Azilsartan, Cozaar, Diovan, Aprovel, Micardis, sartan]
...
- 選擇性阻斷 **AT1 受體** → 阻斷 angiotensin II 的血管收縮與醛固酮分泌;**不影響 bradykinin**。
- 用途與 ACEI 幾乎相同:高血壓第一線、心衰竭、**糖尿病/慢性腎臟病合併蛋白尿**;常用於**因乾咳無法耐受 ACEI** 者。
...
- 小動脈與靜脈擴張 → 周邊阻力 ↓、血壓 ↓。
- 醛固酮 ↓ → 排鈉、保鉀。
- 腎絲球出球小動脈擴張 → 腎絲球內壓與蛋白尿 ↓。
- 不論 angiotensin II 是經 ACE 或其他酵素(如 chymase)產生都能阻斷,阻斷較「完整」。
- 血中 angiotensin II 反而上升,可刺激未被阻斷的 AT2 受體(理論上有血管擴張作用)。

### 與 ACEI 比較 {#vs-acei}

| 項目 | ACEI(-pril) | ARB(-sartan) |
|---|---|---|
| 作用點 | ACE 酵素(AI → AII) | AT1 受體 |
...
| 藥物 | 特色 |
|---|---|
| **Losartan**(Cozaar) | 第一個 ARB;有促尿酸排出作用 |
| **Valsartan**(Diovan) | 與 sacubitril 組成 ARNI(Entresto) |
| **Irbesartan**(Aprovel) | 糖尿病腎病變研究證據 |
| Candesartan、Telmisartan(Micardis)、Olmesartan、Azilsartan | 一天一次;telmisartan 半衰期最長 |

台灣常見與 amlodipine 或 hydrochlorothiazide 的**單一複方錠**(single-pill combination),可提高遵從性。
...
- **糖尿病腎病變、CKD 合併白蛋白尿**。
- 心肌梗塞後左心室功能不良(ACEI 無法耐受時)。

### 轉換成 ARNI {#switch-to-arni}

- **ARB → ARNI**:停 ARB,下一個給藥時間直接改 ARNI 即可(不需空窗)。
- **ACEI → ARNI**:須停 ACEI **至少 36 小時**,否則 bradykinin 分解被雙重抑制(neprilysin 也分解 bradykinin)→ 血管性水腫。
- ARNI 已含 valsartan,**不可再加 ARB 或 ACEI**。
...
- **ACEI、aliskiren**:不併用(見上)。
- 保鉀利尿劑、鉀補充劑、含鉀代鹽、trimethoprim:高血鉀。
- **NSAIDs**:降壓效果 ↓、腎損傷風險 ↑。
- **鋰鹽**:血鋰上升(與 ACEI 相同)。
- 利尿劑:加成降壓,常做成複方。
